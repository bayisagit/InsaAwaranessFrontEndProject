...
# 1. Add state
content = content.replace(" const [filterSearchTerm, setFilterSearchTerm] = useState('');", " const [filterSearchTerm, setFilterSearchTerm] = useState('');\n const [viewingResource, setViewingResource] = useState<Resource | null>(null);")

# 2. Make title clickable
td_target = '<td className="px-6 py-4 font-medium text-foreground">{r.title}</td>'
td_replace = """<td className="px-6 py-4 font-medium text-foreground">
...
content = re.sub(r'(\s*</Modal>\s*</div>\s*);\s*}\s*$', r'\1' + modal_code, content)
