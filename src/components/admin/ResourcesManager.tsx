...
export function ResourcesManager({ lockedCourseId }: ResourcesManagerProps) {
 ...
 return (
 <div className={lockedCourseId ? '' : 'min-h-screen bg-muted pb-20'}>
 {!lockedCourseId && (
 ...
 <p className="text-muted-foreground">Manage and publish learning resources.</p>
 </div>
 ...
 <div className="mb-6">
 <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-3 border-b border-border pb-2">Organization</h3>
 ...
 />
 <span className="text-sm text-muted-foreground group-hover:text-foreground transition-colors leading-tight">{org.name}</span>
 </label>
 ))}
 </div>
 </div>
 ...
 <div className="mb-8">
 <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-3 border-b border-border pb-2">Status</h3>
 ...
 />
 <span className="text-sm text-muted-foreground group-hover:text-foreground transition-colors capitalize">{status}</span>
 </label>
 ))}
 </div>
 </div>
 ...
 <textarea className="w-full px-4 py-2.5 bg-card border border-border rounded-xl text-sm text-foreground placeholder:text-muted-foreground focus:ring-2 focus:ring-primary/20 outline-none min-h-[100px] resize-y" placeholder="Enter resource description or content..." value={form.content} onChange={e => setForm({ ...form, content: e.target.value })} disabled={isActionLoading} />
 </div>
 <CloudinaryUpload
 label="Resource File"
 resourceType="auto"
 ...
 <tr><td colSpan={5} className="px-6 py-8 text-center text-muted-foreground">No resources found matching your criteria.</td></tr>
 ) : filteredResources.map(r => (
 <tr key={r.id} className="hover:bg-muted transition-colors">
 <td className="px-6 py-4 font-medium text-foreground">
 <button onClick={() => setViewingResource(r)} className="text-left hover:text-primary hover:underline transition-colors focus:outline-none cursor-pointer">
 {r.title}
 </button>
 </td>
 <td className="px-6 py-4 text-muted-foreground capitalize">{r.category}</td>
 <td className="px-6 py-4 truncate max-w-[150px]">{getOrgName(r.organization)}</td>
 <td className="px-6 py-4">
 <span className={`px-2 py-1 rounded-full text-[10px] uppercase font-bold ${r.status === 'published' ? 'bg-green-50 text-green-600' : r.status === 'submitted' ? 'bg-blue-50 text-blue-600' : r.status === 'archived' ? 'bg-muted/50 text-muted-foreground' : 'bg-yellow-50 text-yellow-600'
 }`}>
 {r.status}
 </span>
 </td>
 ...
 <button onClick={() => handleSubmitConfirm(r.id)} disabled={isActionLoading} className="px-2 py-1.5 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 hover:text-indigo-700 rounded-lg text-xs font-bold transition-colors mr-2 cursor-pointer">Submit</button>
 )}
 {r.status === 'submitted' && user?.role === 'course_provider' && (
 <button onClick={() => handleWithdrawConfirm(r.id)} disabled={isActionLoading} className="px-2 py-1.5 bg-yellow-50 text-yellow-600 hover:bg-yellow-100 hover:text-yellow-700 rounded-lg text-xs font-bold transition-colors mr-2 cursor-pointer">Withdraw</button>
 )}
 {r.status === 'submitted' && user?.role === 'super_admin' && (
 <>
 <button onClick={() => handleApprove(r.id)} disabled={isActionLoading} className="px-2 py-1.5 bg-green-50 text-green-600 hover:bg-green-100 hover:text-green-700 rounded-lg text-xs font-bold transition-colors mr-2 cursor-pointer">Approve</button>
 <button onClick={() => openRejectModal(r.id)} disabled={isActionLoading} className="px-2 py-1.5 bg-red-50 text-red-600 hover:bg-red-100 hover:text-red-700 rounded-lg text-xs font-bold transition-colors mr-2 cursor-pointer">Reject</button>
 </>
 )}
 <button onClick={() => openModal(r)} className="px-3 py-1.5 bg-blue-50 text-blue-600 hover:bg-blue-100 hover:text-blue-700 rounded-lg text-xs font-bold transition-colors cursor-pointer">Edit</button>
 <button onClick={() => handleDelete(r.id)} className="ml-2 text-red-500 hover:text-red-700 font-medium transition-colors cursor-pointer">Delete</button>
 </td>
 </tr>
 ))}
 </tbody>
 ...
 <span className="text-sm text-muted-foreground">Showing {resources.length} of {totalCount} resources</span>
 ...
 <p className="text-sm text-muted-foreground">Provide a reason for rejecting this resource. The author will see this feedback.</p>
 <div>
