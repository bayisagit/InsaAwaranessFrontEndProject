                                        ...
                                        <tr key={r.id} className="hover:bg-muted transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-right whitespace-nowrap">
                                                {r.status === 'draft' && user?.role === 'course_provider' && (
                                                    <button onClick={() => handleSubmitConfirm(r.id)} disabled={isActionLoading} className="px-2 py-1.5 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 hover:text-indigo-700 rounded-lg text-xs font-bold transition-colors mr-2">Submit</button>
                                                )}
                                                {r.status === 'submitted' && user?.role === 'course_provider' && (
                                                    <button onClick={() => handleWithdrawConfirm(r.id)} disabled={isActionLoading} className="px-2 py-1.5 bg-yellow-50 text-yellow-600 hover:bg-yellow-100 hover:text-yellow-700 rounded-lg text-xs font-bold transition-colors mr-2">Withdraw</button>
                                                )}
                                                {r.status === 'submitted' && user?.role === 'super_admin' && (
                                                    <>
                                                    <button onClick={() => handleApprove(r.id)} disabled={isActionLoading} className="px-2 py-1.5 bg-green-50 text-green-600 hover:bg-green-100 hover:text-green-700 rounded-lg text-xs font-bold transition-colors mr-2">Approve</button>
                                                    <button onClick={() => openRejectModal(r.id)} disabled={isActionLoading} className="px-2 py-1.5 bg-red-50 text-red-600 hover:bg-red-100 hover:text-red-700 rounded-lg text-xs font-bold transition-colors mr-2">Reject</button>
                                                    </>
                                                )}
